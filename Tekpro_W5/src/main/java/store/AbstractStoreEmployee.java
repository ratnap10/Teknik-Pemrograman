...
public abstract class AbstractStoreEmployee implements Employee, Store {
    ...
    @Override
    public double evaluateStoreQuality(){
        if(getNumberOfHoursWorked() > 20 && calculatePay() < 150) {
            return 1.0;
        }else if((getNumberOfHoursWorked() > 15) && calculatePay() < 200) {
            return 2.5;
        }else{
            return 5.0;
        }
    ...
    public abstract boolean isTired();
}
